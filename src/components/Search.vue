...
export default{
        ...
        methods:{
            ...
            getResultMovie(query) {
                ...
                axios.get('https://api.themoviedb.org/3/search/multi?'+this.apiKey+'&language=en-US&page=1&include_adult=false&query='+ query).then(response => { 
...
<template>
    ...
    <div class="search-list">
        <div v-for='result in results' :key='result' class="search-item">
                ...
                <p v-if="result.media_type!='person'" style="color:#fff">{{type=result.media_type}}</p>
